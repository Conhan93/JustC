...
{
    /*
        All the vector assignments at the end of the pdf
        in order.
    */
    std::string buffer; // for input
    std::vector<int> vec; // THE vector

    std::cout << "Enter nr of values you'd like: ";
    std::getline(std::cin,buffer);
    auto nrValues{stoi(buffer)};

    vec.resize(nrValues);
    /*
        resize is a class method for vector(and alot of other containers)
        that "builds" out the vector so that it can contain "nrValues"
        amount of members, it also supports an additional argument
        for initialization of those members if you'd like.

        I used resize so that the vector grows to the size given
        and so that I can fill that size using the generate function
    */

    auto first{0};

    std::generate(begin(vec),end(vec),[&first]() {return first++;});

    /*
        Generate is used to fill the vector, it takes a start position
        and an end position(using iterators) as arguments.

        the last argument is a lambda function.
          [] == is used to capture local variables
            captures first by reference(using &)
          () == arguments taken from caller.
            empty because we're not taking arguments from
            generate
          {} == the actual function for the lambda
            returns first and then increments it

        effect:
            Fills the vector with values 0,1,2,...,nrValues
    */

    // sorts the vector from begin iterator to end iterator
    std::sort(begin(vec),end(vec));

    std::cout << "Largest element is: " << *(end(vec)-1) << std::endl
    << "Three largest elements are: ";
    /*
        It's possible to move a pointer around in memory by
        adding or subtracting integers from the pointer.

        So to access the last value of the sorted array(largest value)
        we fetch the position just after the last pos in the vector
        using end(vec)(vec.end() has the same effect)

        we subtract the end iterator(pointer!) by 1
        which will move the pointer back one position
        in the vector.
        making it point to the last value in the vector
    */


    // for each of the last 3 elements, print value and space
    std::for_each(vec.end()-3,vec.end(),
    [] // captures no local variables
    (int current) // taking arguments from caller(int from vec)
    {std::cout << current << ' ';} // function/action done per call
    );

    std::cout << "Second largest element is: " << *(vec.end()-1) << std::endl;

    int nrMaxes{0};

    std::cout << "Enter number of largest values to view" <<
    "must be less than vector size(" << vec.size() << "): ";
    std::getline(std::cin,buffer);
    nrMaxes = stoi(buffer);
    ...
    if(nrMaxes < vec.size())
      for(auto it = vec.end()-nrMaxes ; it < vec.end() ; it++)
          std::cout << *it << " ";
    /*
        C-style for loop using iterators.
        auto it(iterator) = vec.end() - nrMaxes
          sets it starting at nrMaxes positions from the end
          of the vector
        it < vec.end()
          the conditional check of the for loop,
          keep going till this is false
          equivalent to i < vec.size()
        it++
          advances the pointer to the next member
          in the vector
        *it
          dereferences the iterator(POINTER!)
          allows access to the value pointer to
          by "it"
    */
    std::cout << "\nSmallest integers in vector: " << *vec.begin()
    << " and " << *vec.begin() + 1;

    // seed rand
    srand(time(NULL));

    // fill vector with values generated by lambda
    std::generate(vec.begin(),vec.end(),
    [] // captures no local variables
    () // uses no arguments from callee
    { return (rand() % 100) + 1;} // returns random number from 1 - 100 to func
    );

    // sort vector
    sort(begin(vec), end(vec));

    std::cout << "\nAt least 2 greater elements" << '\n';

    // c-style loop, see above
    for(auto it = vec.begin(); it < end(vec)-2 ; it++)
      std::cout << *it << ' ';


    int Mostvalue{vec.front()}, prevVal{Mostvalue};
    int highestCount = -1;
    int currCount = 0;
    // range based for loop, like in Python "for item in list"
    for(auto number : vec)
    {
      // if same value
      if(prevVal == number) currCount++;
      else // if not
      {
          // if higher count, swap
          if(currCount > highestCount)
          {
            highestCount = currCount;
            Mostvalue = prevVal;
          }
          // change prevVal and reset count
          prevVal = number;
          currCount = 0;
      }
    }

    std::cout << "\nMost occuring number is " << Mostvalue <<
    " occuring " << highestCount << " times" << '\n';
}
